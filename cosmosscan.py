...
ascii_banner = pyfiglet.figlet_format("COSMOS VULNERABILITY SCANNER")
print(ascii_banner)
...
def check_internet():
    os.system('ping -c1 github.com > cs_net 2>&1')
    if "0% packet loss" in open('cs_net').read():
        val = 1
    else:
        val = 0
    os.system('rm cs_net > /dev/null 2>&1')
    return val
...
# CosmosScan Help Context
def helper():
        print(bcolors.OKBLUE+"Information:"+bcolors.ENDC)
        print("------------")
        print("\t./cosmosscan.py example.com: Scans the domain example.com.")
        print("\t./cosmosscan.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests.")
        print("\t./cosmosscan.py example.com --nospinner: Disable the idle loader/spinner.")
        print("\t./cosmosscan.py --update   : Updates the scanner to the latest version.")
        print("\t./cosmosscan.py --help     : Displays this help context.")
        print(bcolors.OKBLUE+"Interactive:"+bcolors.ENDC)
        print("------------")
        print("\tCtrl+C: Skips current test.")
        print("\tCtrl+Z: Quits CosmosScan.")
...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t"+ bcolors.BG_ERR_TXT+"CosmosScan received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC)
            sys.exit(1)
...
tool_cmd   = [
                ...
                #2
                ["wget -O /tmp/Cosmosscan_temp_aspnet_config_err --tries=1 ","/%7C~.aspx"],

                #3
                ["wget -O /tmp/cosmosscan_temp_wp_check --tries=1 ","/wp-admin"],

                #4
                ["wget -O /tmp/cosmosscan_temp_drp_check --tries=1 ","/user"],

                #5
                ["wget -O /tmp/cosmosscan_temp_joom_check --tries=1 ","/administrator"],
                ...
                #75
                ["wget -O /tmp/cosmosscan_temp_aspnet_elmah_axd --tries=1 ","/elmah.axd"],
                ...
                #78
                ["wapiti "," -f txt -o cosmosscan_temp_wapiti"],
...
if args_namespace.help or (not args_namespace.update \
    ...
    print("CosmosScan is updating....Please wait.\n")
    spinner.start()
    # Checking internet connectivity first...
    cs_internet_availability = check_internet()
    if cs_internet_availability == 0:
        ...
        spinner.stop()
        sys.exit(1)
    cmd = 'sha1sum cosmosscan.py | grep .... | cut -c 1-40'
    oldversion_hash = subprocess.check_output(cmd, shell=True)
    oldversion_hash = oldversion_hash.strip()
    os.system('wget -N https://raw.githubusercontent.com/skavngr/cosmosscan/master/cosmosscan.py -O cosmosscan.py > /dev/null 2>&1')
    ...
    if oldversion_hash == newversion_hash :
        clear()
        print("\t"+ bcolors.OKBLUE +"You already have the latest version of CosmosScan." + bcolors.ENDC)
    else:
        clear()
        print("\t"+ bcolors.OKGREEN +"CosmosScan successfully updated to the latest version." +bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
    ...
    while (cs_avail_tools < len(tools_precheck)):
        precmd = str(tools_precheck[cs_avail_tools][arg1])
        try:
            p = subprocess.Popen([precmd], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,shell=True)
            output, err = p.communicate()
            val = output + err
        except:
            print("\t"+bcolors.BG_ERR_TXT+"CosmosScan was terminated abruptly..."+bcolors.ENDC)
            sys.exit(1)
        
        # If the tool is not found or it's part of the --skip argument(s), disabling it
        if b"not found" in val or tools_precheck[cs_avail_tools][arg1] in args_namespace.skip :
            if b"not found" in val:
                print("\t"+bcolors.OKBLUE+tools_precheck[cs_avail_tools][arg1]+bcolors.ENDC+bcolors.BADFAIL+"...unavailable."+bcolors.ENDC)
            elif tools_precheck[cs_avail_tools][arg1] in args_namespace.skip :
                print("\t"+bcolors.OKBLUE+tools_precheck[cs_avail_tools][arg1]+bcolors.ENDC+bcolors.BADFAIL+"...skipped."+bcolors.ENDC)
            
            for scanner_index, scanner_val in enumerate(tool_names):
                if scanner_val[2] == tools_precheck[cs_avail_tools][arg1]:
                    scanner_val[3] = 0 # disabling scanner as it's not available.
                    unavail_tools_names.append(tools_precheck[cs_avail_tools][arg1])

        else:
            print("\t"+bcolors.OKBLUE+tools_precheck[cs_avail_tools][arg1]+bcolors.ENDC+bcolors.OKGREEN+"...available."+bcolors.ENDC)
        cs_avail_tools = cs_avail_tools + 1
        clear()
    unavail_tools_names = list(set(unavail_tools_names))
    if len(unavail_tools_names) == 0:
        print("\t"+bcolors.OKGREEN+"All Scanning Tools are available. Complete vulnerability checks will be performed by CosmosScan."+bcolors.ENDC)
    else:
        print("\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable or will be skipped. CosmosScan will still perform the rest of the tests. Install these tools to fully utilize the functionality of CosmosScan."+bcolors.ENDC)
    ...
    while(tool < len(tool_names)):
        ...
        if tool_names[tool][arg4] == 0:
            ...
            rs_skipped_checks = cs_skipped_checks + 1
            tool = tool + 1
            continue
        try:
            spinner.start()
        ...
        temp_file = "/tmp/cosmosscan_temp_"+tool_names[tool][arg1]
        ...
        if runTest == 1:
                ...
                cs_total_elapsed = cs_total_elapsed + elapsed
                ...
                print("\n")
                #clear()
                cs_tool_output_file = open(temp_file).read()
                if tool_status[tool][arg2] == 0:
                    if tool_status[tool][arg1].lower() in cs_tool_output_file.lower():
                        ...
                        cs_vul_list.append(tool_names[tool][arg1]+"*"+tool_names[tool][arg2])
                else:
                    if any(i in cs_tool_output_file for i in tool_status[tool][arg6]):
                ...
                print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit CosmosScan.\n" + bcolors.ENDC)
                cs_skipped_checks = cs_skipped_checks + 1
    ...
    if len(cs_vul_list)==0:
        print("\t"+bcolors.OKGREEN+"No Vulnerabilities Detected."+bcolors.ENDC)
    else:
        with open("RS-Vulnerability-Report", "a") as report:
            while(cs_vul < len(cs_vul_list)):
                vuln_info = cs_vul_list[cs_vul].split('*')
                report.write(vuln_info[arg2])
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/cosmosscan_temp_"+vuln_info[arg1]
            ...
            print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+"`CS-Vulnerability-Report`"+bcolors.ENDC+" is available under the same directory CosmosScan resides.")

        report.close()
    # Writing all scan files output into CS-Debug-ScanLog for debugging purposes.
    for file_index, file_name in enumerate(tool_names):
        with open("CS-Debug-ScanLog", "a") as report:
            try:
                with open("/tmp/cosmosscan_temp_"+file_name[arg1], 'r') as temp_report:
                        data = temp_report.read()
    ...
    print("\tTotal Number of Vulnerabilities Detected    : "+bcolors.BOLD+bcolors.BADFAIL+str(len(cs_vul_list))+bcolors.ENDC)
    print("\tTotal Time Elapsed for the Scan             : "+bcolors.BOLD+bcolors.OKBLUE+display_time(int(cs_total_elapsed))+bcolors.ENDC)
    print("\n")
    print("\tFor Debugging Purposes, You can view the complete output generated by all the tools named "+bcolors.OKBLUE+"`CS-Debug-ScanLog`"+bcolors.ENDC+" under the same directory.")
    print(bcolors.BG_ENDL_TXT+"[ Report Generation Phase Completed. ]"+bcolors.ENDC)

    os.system('setterm -cursor on')
    os.system('rm /tmp/cosmosscan_te* > /dev/null 2>&1') # Clearing previous scan files
